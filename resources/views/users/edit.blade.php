...
@section('content')
    <div class="save_div">
        <div class="col-12 d-flex justify-content-start">
            <form action="{{ route('update', $user->id) }}" class="d-flex flex-column bg-light p-4 rounded-4 w-100 h-50 gap-4"
                method="post">
                @method('PATCH')
                @csrf
                <div class="form_top d-flex justify-content-center">
                    <div class="form-group flex-grow-2 me-2 d-flex">
                        ...
                        <input type="text" class="form-control rounded-5 mx-3" name="name"
                            value="{{ $user->name }}" />
                    </div>

                    <div class="form-group flex-grow-2 me-2 d-flex">
                        <label for="email">Kullanıcı E-Posta*</label>
                        <input type="text" class="form-control rounded-5 mx-3" name="email"
                            value="{{ $user->email }}" />
                    </div>
